...
- **B1.3b built, needs your on-device test before it's marked ✅.** Home now has a header icon button (top-right, next to the readiness ring — a circular "↻" button) that opens a confirm sheet ("Reschedule my week") and, on confirm, calls the B1.3a engine to re-lay every upcoming session. **Test steps:** open the Schedule tab → tap the ↻ button next to the readiness ring → confirm "Reschedule my week" → you should see an alert reporting how many workouts moved, and the calendar/week view should reflect the new times. Also try it with airplane mode on (should show a "No connection" alert, not a crash or silent no-op) and try double-tapping the button quickly (should not double-run). Report PASS/FAIL using prompt ④ from `EXECUTION.md` §8.
- **Claude's next code batch (after B1.3b is confirmed ✅):** **B1.4 — "Lacking time? 15-min swap" on Home** (S, unblocked, doesn't depend on B0.2). If B1.3b passes, B1.3 is fully ✅ and **B2.1** (re-fence Pro around scheduling) also becomes buildable — either is reasonable to pick up next; B1.4 is smaller and keeps M1 moving, B2.1 starts monetizing the wedge sooner.
- **Handoff note (2026-07-16):** **B1.3b shipped** (this session): header button + `OptionSheet` 2-tap confirm on `(tabs)/index.tsx`, calling `reschedule.rescheduleWholeWeek` behind a single-flight guard; success/empty/offline all resolve to a plain `Alert` (no toast system exists in the app, so this matches the file's existing error-surfacing convention); fires `week_reschedule_used` (moved, total) ahead of B2.2's paywall trigger; invalidates `scheduled_workouts`/`missed_workouts` so the UI reflects the new slots. `tsc` clean, 143 tests green (no new tests — this batch is UI wiring over the already-unit-tested B1.3a engine). Docs: `ARCHITECTURE.md` reschedule section extended. **Not yet done:** on-device test (row is 🔍, not ✅) — see the test steps above. Prior handoff carried forward: B0.2 (Google Calendar API scope) still founder-blocked/in-flight verification — do not add scopes mid-review; B1.5 (multi-calendar) stays deferred behind it.
...
| B1.3b | "Reschedule my whole week" — UI + guards | 🔍 | Subscription Value | `(tabs)/index.tsx` (header button + `OptionSheet` confirm), `lib/analytics.ts` (`week_reschedule_used`) | One tap re-slots the week; offline/2-tap guards; calendar re-synced *(built; needs on-device test)* |
