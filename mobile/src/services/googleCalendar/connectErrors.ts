...
export function friendlyConnectError(code?: string): string {
  switch (code) {
    ...
    case 'identity_taken': return `That Google account already has its own ${BRAND_NAME} login. To use it, sign out and sign back in with Google — or connect your Device Calendar instead.`
    case 'link_unavailable': return 'Google Calendar can’t be attached to this account yet — the device calendar works today.'
    case 'session_switched': return `That Google account doesn’t match your ${BRAND_NAME} account. Try again with the same account.`
